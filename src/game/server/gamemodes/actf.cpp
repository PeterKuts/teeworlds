...
CGameControllerACTF::CGameControllerACTF(class CGameContext *pGameServer)
...
bool CGameControllerACTF::OnEntity(int Index, vec2 Pos)
{
...
int CGameControllerACTF::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int WeaponID)
{
...
void CGameControllerACTF::DoWincheck()
{
...
bool CGameControllerACTF::CanBeMovedOnBalance(int ClientID)
{
...
void CGameControllerACTF::Snap(int SnappingClient)
{
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		if(F->m_pCarryingCharacter)
		{
			...
			if(m_apFlags[fi^1] && m_apFlags[fi^1]->m_AtStand)
			{
				...
				{
					// CAPTURE! \o/
					m_aTeamscore[fi^1] += g_Config.m_ActfCaptureScore;
					F->m_pCarryingCharacter->GetPlayer()->m_Score += g_Config.m_ActfPlayerCaptureScore;
			...
			for(int i = 0; i < Num; i++)
			{
				...
				if(apCloseCCharacters[i]->GetPlayer()->GetTeam() == F->m_Team)
				{
					...
					if(!F->m_AtStand)
					{
						...
						pChr->GetPlayer()->m_Score += g_Config.m_ActfPlayerReturnScore;
                        m_aTeamscore[fi^1] += g_Config.m_ActfReturnScore;
				...
				else
				{
					...
					if(F->m_AtStand)
					{
						m_aTeamscore[fi^1] += g_Config.m_ActfGrabScore;
						F->m_GrabTick = Server()->Tick();
					}

					F->m_AtStand = 0;
					F->m_pCarryingCharacter = apCloseCCharacters[i];
					F->m_pCarryingCharacter->GetPlayer()->m_Score += g_Config.m_ActfPlayerGrabScore;
